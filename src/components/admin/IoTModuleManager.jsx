...
const IoTModuleManager = () => {
  ...
  useEffect(() => {
    if (modalType === "assign" && parcelaId) {
      const loadParcelaDetails = async () => {
        ...
        if (field) {
          setParcelaDims({
            width: parseFloat(field.ancho),
            length: parseFloat(field.largo),
          });
        }
        ...
        try {
          ...
          if (field) {
            const rec = calculateRecommendedPosition(
              field.ancho,
              field.largo,
              iotsList
            );
  ...
  const filteredIots = iots.filter(
    (iot) =>
                ...
                {filteredIots.map((iot, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">
                      {iot.fechaCreacion
                        ? new Date(iot.fechaCreacion).toLocaleDateString(
                            "es-MX"
                          )
                        : "—"}
                    </td>
                    <td className="px-4 py-3">
                      {iot.id_parcela ? (
                        (() => {
                          const parcela = fields.find(
                            (p) => p.id_parcela === iot.id_parcela
                          );
                          return (
                    ...
                    <td className="px-4 py-3 text-center space-x-2">
                      ...
                      <Button
                        ...
                        className="px-3 py-1.5 text-sm bg-red-500 text-white hover:bg-red-600"
                      >
                        Eliminar
                      </Button>
                    </td>
                ...
                >
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="text-sm">
                      ...
                      <p className="font-medium">{iot.descripcion || "—"}</p>
                    </div>
        ...
        {modalType === "details" && selectedIot && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-3 text-sm bg-gray-50 p-4 rounded-lg">
              <p>
                <strong>ID:</strong> #{selectedIot.id_iot}
              </p>
              <p>
                <strong>Desc:</strong> {selectedIot.descripcion}
              </p>
              <p>
                <strong>Coords:</strong> X: {selectedIot.coordenada_x || "N/A"},
                Y: {selectedIot.coordenada_y || "N/A"}
              </p>
            </div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      backgroundImage:
                        "linear-gradient(#6FA575 1px, transparent 1px), linear-gradient(90deg, #6FA575 1px, transparent 1px)",
                      backgroundSize: "20px 20px",
                    }}
                  ></div>

                  {/* Ejes */}
                  <div className="absolute bottom-1 left-1 text-[10px] text-gray-500 bg-white/80 px-1 rounded pointer-events-none">
                    (0,0)
                  </div>
                  <div className="absolute bottom-1 right-1 text-[10px] text-gray-500 pointer-events-none">
                    X ({parcelaDims.width}m)
                  </div>
                  <div className="absolute top-1 left-1 text-[10px] text-gray-500 pointer-events-none">
                    Y ({parcelaDims.length}m)
                  </div>
                  ...
                  {existingParcelaIots.map((exist, idx) => {
                    ...
                    const bottom =
                      (exist.coordenada_y / parcelaDims.length) * 100;
                    return (
                      <div
                        key={idx}
                        className="absolute w-6 h-6 bg-gray-400 rounded-full border-2 border-white shadow-sm flex items-center justify-center text-white z-10 pointer-events-none"
                        style={{
                          left: `${left}%`,
                          bottom: `${bottom}%`,
                  ...
                  {recommendedCoords && (
                    <div
                      className="absolute w-8 h-8 bg-[#6DA544]/40 rounded-full border-2 border-[#6DA544] border-dashed flex items-center justify-center text-white z-0 pointer-events-none animate-pulse"
                      style={{
                        left: `${
                          (recommendedCoords.x / parcelaDims.width) * 100
                        }%`,
                        bottom: `${
                          (recommendedCoords.y / parcelaDims.length) * 100
                        }%`,
                        transform: "translate(-50%, 50%)",
                      }}
                    >
                  ...
                  <div
                    className="absolute w-8 h-8 bg-[#eab308] rounded-full border-2 border-white shadow-lg flex items-center justify-center text-white z-20 transition-all duration-200 ease-out"
                    style={{
                ...
                <div className="flex items-center justify-between mt-2 text-sm">
                  <div className="flex gap-3 text-xs text-gray-600">
                    ...
                    <span className="flex items-center gap-1">
                      <div className="w-2 h-2 bg-[#6DA544]/50 rounded-full border border-dashed border-[#6DA544]"></div>{" "}
                      Sugerido
                    </span>
                    <span className="flex items-center gap-1">
                  ...
                  <div className="flex gap-2">
                    <div className="flex items-center gap-1">
                      ...
                      <input
                        type="number"
                        className="w-16 p-1 border rounded bg-gray-50 text-right"
                        value={coords.x}
                        onChange={(e) =>
                          setCoords({
                            ...coords,
                ...
                {recommendedCoords && (
                  <p
                    className="text-xs text-green-600 text-right mt-1 cursor-pointer hover:underline"
                    onClick={() => setCoords(recommendedCoords)}
                  >
                    Usar recomendación: ({recommendedCoords.x},{" "}
                    {recommendedCoords.y})
                  </p>
                )}
              </div>
